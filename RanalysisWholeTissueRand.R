...
mydata <- read.csv(paste(getwd(),"/",number,"_Converted_coordinates_DIAMETER_",diameter,"Rand.csv",sep=""))
...
myMax <- max(myArea)
mv_simpl <- st_simplify(subset(polyList$geometry, st_area(polyList$geometry) > myMax*0.05), preserveTopology = FALSE, dTolerance = 180)
plot(mv_simpl)
#No changes
...
for (i in 1:length(mv_simpl)){
  ...
  if((mypatternStr$n + mypatternNeg$n + mypatternPos$n) > 700){
    ...
    write.table(myRhoP, file = paste(i,"_myRhoPRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
    write.table(myRho, file = paste(i,"_myRhoNRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
    
    nn <- nncross(mypatternPos, mypatternStr)$dist
    mm <- nncross(mypatternNeg, mypatternStr)$dist
    write.table(nn, file = paste(i,"_myNNPRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
    write.table(mm, file = paste(i,"_myNNNRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
    ...
    write.table(myenvp, file = paste(i,"_myenvRDFPRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
    write.table(myenv, file = paste(i,"_myenvRDFNRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
    ...
    write.table(myenvpL, file = paste(i,"_myenvLcrossPRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
    write.table(myenvL, file = paste(i,"_myenvLcrossNRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
    ...
    write.table(myenvpJ, file = paste(i,"_myenvJcrossPRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
    write.table(myenvJ, file = paste(i,"_myenvJcrossNRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
    #Gcross #############################################################
    ...
    write.table(myenvpG, file = paste(i,"_myenvGcrossPRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
    write.table(myenvG, file = paste(i,"_myenvGcrossNRand",".csv",sep=""),row.names=FALSE,col.names=TRUE,sep=",")
