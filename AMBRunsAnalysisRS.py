...
for ee in range(Nsteps):
    averNumDiv[ee] = averNumDiv[ee]/(N) 
    averNumDie[ee] = averNumDie[ee]/(N) 
    averTotalGrid[ee] = averTotalGrid[ee]/(N) - 1094# len(StromaStr)/2
...
for fd in range(500):
    tmp = []
    for hh in range(500):
        tmp.append(sdTotalGrid[hh][fd])
    SDaver.append(statistics.stdev(tmp))
